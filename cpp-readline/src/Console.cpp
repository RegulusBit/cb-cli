...
namespace CppReadline {
    ...
    Console::Console(std::string const& greeting)
        : pimpl_{ new Impl{ greeting } }, command_description()
    {
    ...
    void Console::registerCommand(const std::string & s, CommandFunction f, std::string description) {
        pimpl_->commands_[s] = f;
        command_description.insert({s, description});
    }
